...
Rather than using SQLite triggers (fragile, hard to maintain, no access to transaction context), endb-lite would use Turso's CDC stream to populate history tables. When a write hits `_events_current`:
...
Tables in endb-lite do not require `CREATE TABLE` with predefined columns. Instead:
...
Following Endatabas, endb-lite's parser would support document literal syntax:
...
In endb-lite, `UPDATE` and `DELETE` are non-destructive by default. They create new versions.
